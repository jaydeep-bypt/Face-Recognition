...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        for person_name, data in known_people.items():
            face_distances = face_recognition.face_distance(data['faces'], face_encoding)
            min_face_distance = min(face_distances)
            current_match_percentage = (1 - min_face_distance) * 100
        ...
        # Step 6: Set conditions based on match percentage and minimum distance
        if match_percentage >= 60 and min_distance < 0.6:
            # Recognize the face
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 2)
            font = cv2.FONT_HERSHEY_DUPLEX
