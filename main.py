...
#         # Step 6: Display the results on the frameq
...
while True:
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        for person_name, data in known_people.items():
            matches.extend(face_recognition.compare_faces(data['faces'], face_encoding))
        ...
        if True in matches:
            first_match_index = matches.index(True)
            face_distances = face_recognition.face_distance(known_people[person_name]['faces'], face_encoding)
            percentage_match = (1 - face_distances[first_match_index]) * 100

            if percentage_match > 60:
                name = known_people[person_name]['names'][0]  # Use the folder name as the label
                # Additional information about the match
                match_info = f"Matched with {name} ({percentage_match:.2f}%)"
                print(match_info)
